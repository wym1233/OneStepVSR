...
def parse_args_paired_training(input_args=None):
   ...
   """
    ...
    parser.add_argument("--base_config", default="/code/S3Diff-main/configs/sr.yaml", type=str)
    ...
    parser.add_argument("--pretrained_path", type=str, default='/data/wym123/VSRdataset/S3diffLora/s3diff.pkl')
    parser.add_argument("--de_net_path",default='/code/S3Diff-main/assets/mm-realsr/de_net.pth')
    ...
    # training details
    parser.add_argument("--output_dir", default='/data/wym123/VSRdataset/ckptdir_s3diffinserted')
    ...
    parser.add_argument("--logging_dir", type=str, default='/data/wym123/VSRdataset/ckptdir_s3diffinserted/loggingdir')
...
class PairedDataset(data.Dataset):
        ...
        if 'meta_info_file' in opt:
            with open(opt['meta_info_file'], 'r') as fin:
                for line in fin:
                    ...
                    self.paths.extend([f'{folder}/{i:08d}' for i in range(int(frame_num))])

                # remove the video clips used in validation
                if len(self.paths)==24000:
                    val_partition = ['000', '011', '015', '020']   
                    self.paths = [v for v in self.paths if v.split('/')[0] not in val_partition]

        print('length:'+str(len(self.paths)))
    ...
    def __getitem__(self, index):
        ...
        gt_path = self.opt['gt_path'][0]+self.paths[index]+'.png'
        ...
        img_gt=(img_gt*2-1).clamp(-1,1)
        resizedLR=(resizedLR*2-1).clamp(-1,1)


        return_d = {'gt': img_gt, 'lr':resizedLR.squeeze(),'de':resizedLR_denet.squeeze(),'gt_path':gt_path}
        return return_d
...
def degradation_proc(configs, batch, device, val=False, use_usm=False, resize_lq=True, random_size=False):
    ...
    B=im_gt.shape[0]
    # print(batch['kernel1'].shape)
    # print(batch['kernel2'].shape)
    # print(torch.max(batch['kernel1']),torch.min(batch['kernel1']))
    # print(torch.max(batch['kernel2']),torch.min(batch['kernel2']))
    # torch.Size([4, 21, 21])
    # torch.Size([4, 21, 21])
    # tensor(0.9919, device='cuda:0') tensor(0., device='cuda:0')
    # tensor(0.9960, device='cuda:0') tensor(0., device='cuda:0')
    kernel1 = batch['kernel1'][0:1].repeat(4,1,1)
    kernel2 = batch['kernel2'][0:1].repeat(4,1,1)
    sinc_kernel = batch['sinc_kernel'][0].repeat(4,1,1)
