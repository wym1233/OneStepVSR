...
import model.stableVSR.util.flow_utils as of
...
class StableVSRPipeline(
    ...
    def compute_flops(
        ...
        估算模型总 FLOPs
        Args:
            height, width: 输入图像分辨率
            num_frames: 视频帧数
            num_inference_steps: 扩散步数
        Returns:
            total_flops: 总浮点运算次数 (GFLOPs)
        """
        with torch.no_grad():
            ...
            # 创建虚拟输入
            dummy_latent = torch.randn(1, 7, h_latent, w_latent).to(device)
            dummy_cond = torch.randn(1, 77, 1024).to(device)  # 文本嵌入
            dummy_image = torch.randn(1, 3, h_latent, w_latent).to(device)
            condimg=torch.randn(1, 3, height, width).to(device)
            dummy_timestep = torch.tensor([0],dtype=torch.long).to(device)
            
            # 1. 统计 UNet FLOPs
            unet_flops = FlopCountAnalysis(self.unet, (
                dummy_latent, 
                dummy_timestep, 
                dummy_cond
            )).total()
            ...
            # 2. 统计 ControlNet FLOPs
            if isinstance(self.controlnet, ControlNetModel):
                controlnet_flops = FlopCountAnalysis(self.controlnet, (
                    dummy_latent,
                    dummy_timestep,
                    dummy_cond,
                    condimg
                )).total()
            else:  # MultiControlNet
                controlnet_flops = sum([
                    FlopCountAnalysis(net, (
                        torch.cat([dummy_latent, img], dim=1),
                        dummy_timestep,
                        dummy_cond
                    )).total() for net, img in zip(self.controlnet.nets, [dummy_image])
                ])
            print('controlnet_flops')
            print(controlnet_flops)
            ...
            del self.unet
            del self.controlnet
            dummy_latent1 = torch.randn(1, 4, 180, 180).to(device)
            vae_flops = FlopCountAnalysis(self.vae.decoder, dummy_latent1).total()
            print('vae_flops')
            print(vae_flops)
            ...
            # 计算总 FLOPs (考虑调用次数)
            total_flops = (
                (unet_flops * num_frames * num_inference_steps) +
                (controlnet_flops * (num_frames - 1) * num_inference_steps) +
                (vae_flops * num_frames)
            )
