...
class REDSRecurrentDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['crop']:
            img_gts, img_lqs = paired_random_crop(img_gts, img_lqs, self.opt['gt_size'], scale, img_gt_path)
        if type(img_lqs)==np.ndarray:
            img_gts=[img_gts]
            img_lqs=[img_lqs]
        ...
        img_gts=(2*img_gts-1).clamp(-1,1)
        img_lqs=(2*img_lqs-1).clamp(-1,1)
